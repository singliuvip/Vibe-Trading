...
def create_app():
    ...
    async def account_snapshot(body: dict[str, Any]):
        ...
        try:
            ...
            if connect_result != 0:
                return {
                    "status": "error",
                    "error": f"miniQMT connection returned error code {connect_result}",
    ...
    async def positions(body: dict[str, Any]):
        ...
        try:
            ...
            if trader.connect() != 0:
                return {
                    "status": "error",
                    "error": "miniQMT connection failed",
                    "error_code": "MINIQMT_DISCONNECTED",
                }
        except Exception as exc:
            return {
    ...
    async def orders(body: dict[str, Any]):
        """Get open orders and optionally executions."""
        account_id = str(body.get("account_id", ""))
        account_type = str(body.get("account_type", "STOCK")).upper()
        mini_qmt_path = str(body.get("mini_qmt_path", ""))
        ...
        order_status_map = {
            48: "pending", 49: "pending", 50: "pending",
            53: "partial",
            54: "filled",
            51: "cancelled", 52: "cancelled", 55: "cancelled",
            56: "rejected", 57: "rejected",
        }
    ...
    async def quote(body: dict[str, Any]):
        ...
        if not tick or symbol not in tick:
            return {
                "status": "error",
                "symbol": symbol,
                "error": f"No tick data returned for {symbol}",
    ...
    async def bars(body: dict[str, Any]):
        ...
        period_map = {
            ...
            "1h": "60m", "4h": "240m", "1d": "1d", "1w": "1w", "1M": "1M",
        }
        ...
        if callable(to_dict) and hasattr(df, "columns"):
            try:
                ...
                for row in records:
                    bars_list.append({
                        "t": str(_to_native(row.get("index") or row.get("time") or row.get("datetime") or "")),
            ...
            for i in range(max(0, len(times) - count), len(times)):
                bars_list.append({
                    "t": str(_to_native(times[i] if i < len(times) else "")),
    ...
    async def place_order(body: dict[str, Any]):
        """Place an order (future — currently blocked)."""
        if _is_halted():
            return {"status": "blocked", "decision": "halt", "reason": "kill switch tripped"}
        return {
            "status": "error",
            "error": "Order placement through QMT Bridge is not yet implemented.",
    ...
    async def cancel_order(body: dict[str, Any]):
        """Cancel an order (future — currently blocked)."""
        if _is_halted():
            return {"status": "blocked", "decision": "halt", "reason": "kill switch tripped"}
        return {
            "status": "error",
            "error": "Order cancellation through QMT Bridge is not yet implemented.",
            "error_code": "NOT_IMPLEMENTED",
        }
