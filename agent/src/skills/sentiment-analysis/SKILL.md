...
```
数据源:
- 中文: 东方财富股吧（结构化，get_a_share_social_sentiment 工具）/ 雪球 / 微博 / 微信公众号
- 英文: Twitter(X)/Reddit/Telegram
- 加密: CryptoTwitter/Discord/Telegram群

采集分工:
- 股吧: get_a_share_social_sentiment 工具（结构化帖子元数据，免鉴权，经 eastmoney 节流）
...
A股社媒为中文，VADER/FinBERT 英文模型不适用，采用：

1. **LLM 打分（首选）**：输出单条文本情绪分 `-1.0..1.0`（与 event-driven skill 一致），适合中文财经语境。详见 `social-media-intelligence` skill 第 3.1 节。
2. **规则词典兜底**：LLM 不可用时，用中文财经情绪词典（"涨停/牛市/利好"→正向，"跌停/崩盘/利空"→负向）快速批量打分。

### 评分尺度对照表

单条文本 LLM 打分 `-1.0..1.0`，聚合后映射到 `-100..+100`（与综合情绪评分框架对齐）：
...
聚合公式：`聚合分 = mean(单条 score) × 100`，按作者类别加权（机构:3 / KOL:2 / 散户:1）。
